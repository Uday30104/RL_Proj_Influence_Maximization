...
class Tripling(nn.Module):
    ...
    def __init__(self, embed_dim, sgate_l1_dim, tgate_l1_dim, T, hidden_dims, w_scale, num_communities=0):
        '''
        embed_dim=50 -> 2*50+1
        T: number of layers of tripling
        num_communities: added to support community-aware features
        '''
    ...
    def forward(self, data):
        ...
        # --- NEW: Inject Community Features into Initial Embeddings ---
        if hasattr(data, 'comm') and self.num_communities > 0:
            source_influ = source_influ + F.leaky_relu(self.comm_proj_source(data.comm), 0.2)
            target_influ = target_influ + F.leaky_relu(self.comm_proj_target(data.comm), 0.2)
...
class DeepWalkNeg(nn.Module):
    # ... [DeepWalkNeg implementation remains exactly the same] ...
...
class S2V_DUEL(nn.Module):
    # ... [S2V_DUEL implementation remains exactly the same, typically omitted in basic ToupleGDD usage] ...
