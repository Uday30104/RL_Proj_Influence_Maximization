...
class Environment:
    ...
    def compute_reward(self, S):
        ...
        # 2. Calculate Community Gain (NEW)
        total_comms = set()
        # Check if the graph has the community dictionary we added in graph_utils.py
        if hasattr(self.graph, 'communities') and self.graph.communities:
            for node in S:
                # Add all communities this node belongs to
                total_comms.update(self.graph.communities.get(node, set()))
                
        comm_gain = len(total_comms) - self.prev_comm
        self.prev_comm = len(total_comms)
